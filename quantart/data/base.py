...
class UnpairedImagePaths(Dataset):
    ...
    def __getitem__(self, i):
        example = dict()
        example["image1"], example["image1_path"] = self.preprocess_image(
            self.paths1[i])
        i2 = random.randrange(len(self.paths2))
        example["image2"], example["image2_path"] = self.preprocess_image(
            self.paths2[i2])
...
class PairedImagePaths(Dataset):
    ...
    def __getitem__(self, i):
        example = dict()
        example["image1"] = self.preprocess_image(
            self.labels["file_path_"][i][0])
        example["image2"] = self.preprocess_image(
            self.labels["file_path_"][i][1])
